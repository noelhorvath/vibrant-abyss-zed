# Vibrant Abyss

## Table of contents

- [Description](#description)
- [Features](#features)
- [Screenshots](#screenshots)
- [License](#license)

## Description

**Vibrant Abyss** is a high-contrast **Zed** theme designed for optimal clarity and readability against a true pitch-black background.
It's based on the original [Vibrant Abyss for **Visual Studio Code**](https://github.com/noelhorvath/vibrant-abyss).

## Features

- Dark theme
- Pitch black
- Vibrant colors

## Screenshots

![CSharp](https://github.com/noelhorvath/vibrant-abyss-zed/blob/main/screenshots/csharp.png?raw=true)
</br>
![Rust](https://github.com/noelhorvath/vibrant-abyss-zed/blob/main/screenshots/rust.png?raw=true)
</br>
![Tsx](https://github.com/noelhorvath/vibrant-abyss-zed/blob/main/screenshots/tsx.png?raw=true)

## License

This project is unlicensed. You are free to use, modify, and distribute the
code without any restrictions. Refer to the [LICENSE] file for more details.

[LICENSE]: https://github.com/noelhorvath/vibrant-abyss-zed/blob/main/LICENSE.md
